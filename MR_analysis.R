...
CI_surv_not_merged_with_ref <- fread("surv_DEMENTIA.txt")
...
Panc_Alc_Chr <- fread("Panc_1_bin.tsv")
Panc_nonAlc_Chr <- fread("Panc_2_bin.tsv")
...
for(i in 1:length(exp)){
   for(j in 1:length(out)){
      ...
      unlink(paste("/working/", folder, sep=""), recursive = TRUE)
      unlink(paste("/working/", folder, ".zip", sep=""), recursive = TRUE)
      dir.create(folder)
...
zip(zipfile = "MR_results.zip", '/working/MR_results')
